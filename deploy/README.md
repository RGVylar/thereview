...
```bash
bash <(curl -fsSL https://raw.githubusercontent.com/RGVylar/thereview/main/deploy/create-lxc.sh)
```
...
```bash
CT_ID=201 CT_NAME=thereview-prod DOMAIN=thereview.example.com \
bash <(curl -fsSL https://raw.githubusercontent.com/RGVylar/thereview/main/deploy/create-lxc.sh)
```
...
- `DOMAIN=thereview.example.com` — your domain
...
2. Creates the `thereview` system user and clones the repo to `/opt/thereview`.
...
6. Installs and starts the `thereview-api` systemd unit.
7. Configures Caddy as a local reverse proxy on port 80.

## Cloudflare Tunnel

Point your tunnel hostname at `http://127.0.0.1:80`
