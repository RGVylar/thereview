#!/usr/bin/env bash
# Create and configure a The Review LXC on Proxmox
...
msg "Cloning repo…"
pct exec "$CT_ID" -- rm -rf /tmp/thereview
pct exec "$CT_ID" -- git clone --branch main https://github.com/RGVylar/thereview.git /tmp/thereview

msg "Running install.sh…"
pct exec "$CT_ID" -- env DOMAIN="$DOMAIN" BACKEND_PORT="$BACKEND_PORT" bash /tmp/thereview/deploy/install.sh

echo ""
cat <<EOF
${GRN}✔ The Review LXC ready${NC}
...
Logs:
  pct exec $CT_ID -- journalctl -u thereview-api -f
