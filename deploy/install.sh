#!/usr/bin/env bash
# The Review — LXC installer (Debian 12)
# Usage (inside the LXC, as root):
#   bash -c "$(curl -fsSL https://raw.githubusercontent.com/RGVylar/thereview/main/deploy/install.sh)"
#
...
if [[ -n "$GITHUB_TOKEN" ]]; then
    REPO_URL="${REPO_URL:-https://${GITHUB_TOKEN}@github.com/RGVylar/thereview.git}"
else
    REPO_URL="${REPO_URL:-https://github.com/RGVylar/thereview.git}"
fi
REPO_BRANCH="${REPO_BRANCH:-main}"
APP_DIR="${APP_DIR:-/opt/thereview}"
APP_USER="${APP_USER:-thereview}"

DB_NAME="${DB_NAME:-thereview}"
DB_USER="${DB_USER:-thereview}"
DB_PASS="${DB_PASS:-$(openssl rand -hex 16)}"

DOMAIN="${DOMAIN:-thereview.example.com}"
...
if [[ -f "$APP_DIR/frontend/package.json" ]]; then
    ...
    sudo -u "$APP_USER" bash -c "cd $APP_DIR/frontend && npm install --legacy-peer-deps --silent && npm run build --silent"
    ok "Frontend built"
else
...
install -m 644 "$APP_DIR/deploy/thereview-api.service" /etc/systemd/system/thereview-api.service
...
systemctl enable --now thereview-api.service >/dev/null
systemctl reload caddy
ok "Services up"

cat <<EOF

${GRN}✔ The Review installed${NC}
  ...
  App dir:   $APP_DIR
  Logs:      journalctl -u thereview-api -f
